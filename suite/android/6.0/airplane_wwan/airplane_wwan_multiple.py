...
Add the ConfigParser on 2019/06/12
...
class airplane_wwan_multiple(object):
	...
	def start_airplane(self):
		try:
			...
			self._adb_shell(
				config.get('sn', 'sn'), 'am broadcast -a android.intent.action.AIRPLANE_MODE --ez state true')
			time.sleep(6)
	...
	def stop_airplane(self):
		try:
			...
			self._adb_shell(
				config.get('sn', 'sn'), 'am broadcast -a android.intent.action.AIRPLANE_MODE --ez state false')
			time.sleep(30)

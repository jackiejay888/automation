...
svc wifi disable
/data/testtool/busybox ifconfig eth0 up
sync
sleep 3
#netcfg   

MACaddress=`ifconfig eth0 | /data/testtool/busybox awk 'NR==1{print $5}'`
...
if [ "$Test_res" == "true" ] ; then
  ...
  do
     sleep 5
    IP_ADDRESS=` ifconfig eth0 | /data/testtool/busybox awk '/inet addr/{print substr($2,6)}'`
    netCount=$(($netCount+1))			
  done 
  echo $IP_ADDRESS

...
echo 'Test_Item: sd_info hardware' >> $log_patch/$project_name"_"$fun"_"$now.log &
if [ "$OpenLoop" == "true" ] ; then
...
/data/testtool/busybox fdisk -l /dev/block/mmcblk1
/data/testtool/busybox fdisk -l /dev/block/mmcblk1 >> $log_patch/$project_name"_"$fun"_"$now.log &

sdinfo=`/data/testtool/busybox fdisk -l /dev/block/mmcblk1 |grep Disk |/data/testtool/busybox awk '{print $5}'`

		if [ -z  "$sdinfo" ] ; then
		Test_res=false
		echo "no sd card mounted" 
		echo "no sd card mounted" >> $log_patch/$project_name"_"$fun"_"$now.log &
		fi

...
echo 'Test_Item: mem_info hardware' >> $log_patch/$project_name"_"$fun"_"$now.log &
if [ "$OpenLoop" == "true" ] ; then
...
mem_size=`cat /proc/meminfo | grep "MemTotal:" | busybox awk {'print $2'}`

cat /proc/meminfo
cat /proc/meminfo >> $log_patch/$project_name"_"$fun"_"$now.log &

		if [ $((mem_size)) -ne $(($1)) ] ; then
		Test_res=false
		echo "memory size is wrong:" $cpuinfo "!=" $1
		echo "memory size is wrong:" $cpuinfo "!=" $1 >> $log_patch/$project_name"_"$fun"_"$now.log &
		fi
